...
const ArchiveModal: React.FC<ArchiveModalProps> = ({ onClose, onSelectDate, startDate, currentDate }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
      <div className="bg-[#22223b] w-full max-w-md rounded-2xl border border-[#c9ada7] flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 overflow-y-auto grid grid-cols-3 gap-3">
          {days.reverse().map((day) => (
            <button
              ...
              className="flex flex-col items-center p-3 rounded-xl border border-gray-600 hover:bg-[#4a4e69] transition text-white active:scale-95"
            >
              <span className="text-xs text-gray-400">Day</span>
              <span className="text-lg font-bold">{day.dayNum}</span>
              <span className="text-[10px] opacity-60">{day.date}</span>
            </button>
          ))}
        </div>
      </div>
